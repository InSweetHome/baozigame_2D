import { _decorator, Collider2D, Component, Contact2DType, Input, input, instantiate, Label, Node, Prefab, tween, Vec3 } from 'cc';
...
export class player extends Component {
    ...
    Touch_Start(){
        ...
        this.Arrow_Node.getComponent(Collider2D).on(Contact2DType.BEGIN_CONTACT, this.Arrow_Colide, this) // 箭矢生成后开始监听是否有碰撞
        tween(this.Arrow_Node).to(0.3, {position: new Vec3(25, 80, 0)}).call(
            () =>{// .call都是在箭矢粘在箭靶后发生的事情
                this.Arrow_Node.getComponent(Collider2D).off(Contact2DType.BEGIN_CONTACT, this.Arrow_Colide, this); //当箭矢粘在箭靶上后，表明发射成功，就不需要监听了，注销。
                this.Arrow_Node.setParent(this.Target_Node, true) // 箭矢粘住箭靶后需要跟着箭靶一起旋转
